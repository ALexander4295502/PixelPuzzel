...
struct CanvasViewModel {
    ...
    mutating func addIndexAt(x: Int, y: Int, color: UIColor) {
        let pixelState = PixelState(x: x, y: y, color: color)
        delegate?.addIndex(newPixelState: pixelState)
    }

    mutating func check(x: Int, y: Int) {
        delegate?.checkState(x: x, y: y)
    }
    ...
    mutating func undo() {
        guard let lastDraw = stateHistory.popLast() else {
            return
        }
    ...
    mutating func redo() {
        guard let lastUndoneDraw = undoneChanges.popLast() else {
            return
        }
